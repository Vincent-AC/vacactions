...
unfix_parameters_and_replace_model <- function(model_dir,
                                               model_file,
                                               parameters_to_unfix)
{
  ...
  model <- pharmpy$Model(paste0(model_dir,model_file))
  ...
  model$write(paste0(model_dir,model_file),
              force = T)
  usethis::ui_info(base::paste0("Update of ", model_file,
                                " done"))
}
